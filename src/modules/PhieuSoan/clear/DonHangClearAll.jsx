...
const DonHangClearAll = ({ onClearSuccess }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await donHangService.deleteManyDonHang();
      ...
      setTimeout(() => {
        alert(response?.message || "✅ Đã xóa toàn bộ đơn hàng thành công");
      }, 100);
  ...
  return (
    <>
      <button
        onClick={handleOpen}
        className="flex items-center gap-2 rounded-xl bg-red-600 px-4 py-3 text-sm font-medium text-white shadow-sm hover:bg-red-700 transition"
      >
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 bg-red-50 border border-red-200 rounded-xl">
                ...
                <p className="text-sm text-red-700 mt-1">
                  Tất cả dữ liệu đơn hàng sẽ bị xóa vĩnh viễn.
                </p>
              </div>

              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-700">
                  Vui lòng nhập <span className="font-mono text-red-600">DELETE_ALL</span> để xác nhận:
                </label>
                <input
                  type="text"
                  value={confirmation}
                  ...
                  className="w-full px-4 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent disabled:bg-slate-100 disabled:cursor-not-allowed font-mono"
                />
              </div>
...
export default DonHangClearAll;