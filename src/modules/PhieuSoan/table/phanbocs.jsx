...
import ImportPhanBoCS from "./component/phanbocs/importphanboCS";
import ImportSdTfCS from "./component/phanbocs/importsdtfCS";
import ExportDataloadButtonCS from "./component/phanbocs/exportdataloadCS";
...
// ─── Main Table ───────────────────────────────────────────────────────────────
const PhanBoCSTable = forwardRef((_, ref) => {
  ...
  const dChuyen = useDebounce(filterChuyen);
  ...
  const params = useMemo(
    () => ({
      page,
      limit,
      search: [dTenPhanBo, dSdTf, dMaCH, dTenCH, dChuyen]
        .filter(Boolean)
        .join(" "),
      ...
      endDate: dEndDate,
      startNgayXuLi: dStartNgayXuLi,
      endNgayXuLi: dEndNgayXuLi,
      sku: dSku,
      name: dName,
  ...
  const fetchPhanBoCS = useCallback(async () => {
    setLoading(true);
    setError("");
    try {
      const res = await phanBoCSService.getAllPhanBoCS(params);
      ...
      setError("Không tải được dữ liệu Phân Bổ CS.");
    } finally {
      setLoading(false);
    }
  ...
  const resetFilters = useCallback(() => {
    ...
    setDateRangeXuLi([{ startDate: null, endDate: null, key: "selection" }]);
  }, []);
  ...
  // checkbox + # + ten_phan_bo + sd_tf + mach + tench + chuyen + sku + name + pack + luong_pb + so_thung + ngay_import + ngay_xu_li + trang_thai
  const COL_SPAN = 15;
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-5 shadow-sm">
        <div className="flex flex-col md:flex-row gap-3 flex-wrap">
          ...
          <ImportPhanBoCS onImportSuccess={fetchPhanBoCS} />
          <ImportSdTfCS onImportSuccess={fetchPhanBoCS} />
          <ExportDataloadButtonCS
            selectedRows={selectedRows}
            fileName="phan_bo_cs_dataload"
            onExportSuccess={() => {
              fetchPhanBoCS();
              setSelectedRows([]);
            }}
          ...
          <thead className="sticky top-0 z-10 bg-slate-50/95 backdrop-blur">
            ...
            <tr className="border-b border-slate-200 bg-slate-100/50">
              ...
              <th className="px-3 py-1">
                <FilterInput
                  value={filterChuyen}
                  onChange={mkChange(setFilterChuyen)}
                />
              </th>
              <th className="px-3 py-1">
                <FilterInput
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-center text-slate-600">
                      {row.gia
                        ? Number(row.gia).toLocaleString("vi-VN") + " ₫"
                        : "-"}
                    </td>{" "}
...
PhanBoCSTable.displayName = "PhanBoCSTable";
export default PhanBoCSTable;
