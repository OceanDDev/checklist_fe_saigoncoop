...
const PhieuSoanRow = ({
  ...
  getChanLeBadge,
  onSortChange,
  allRowsBeforeFilter = [], // THÊM PROP MỚI: tất cả rows trước khi filter
}) => {
  const [sortConfig, setSortConfig] = useState({ key: null, direction: null });
  
  // Track checkbox state independent of filtered rows
  const [isCheckboxChecked, setIsCheckboxChecked] = useState(false);

  // Sync checkbox state với selectAll từ parent
  useEffect(() => {
  ...
  // TÍNH TỔNG KIỆN ĐÃ SELECT - Dùng allRowsBeforeFilter thay vì sortedRows
  const selectedSummary = useMemo(() => {
    if (!selectedRows || selectedRows.length === 0) {
      return null;
    }

    // SỬ DỤNG allRowsBeforeFilter để tính tổng tất cả items đã chọn
    // (bao gồm cả những item bị ẩn do filter)
    const dataSource = allRowsBeforeFilter.length > 0 ? allRowsBeforeFilter : sortedRows;
    
    const selectedItems = dataSource.filter((row) =>
      selectedRows.includes(row._id)
    );

    const totalKien = selectedItems.reduce(
      (sum, row) => sum + (Number(row.kien_hang) || 0),
      0
    );
    const totalLuong = selectedItems.reduce(
      (sum, row) => sum + (Number(row.luong) || 0),
      0
    );
  ...
  }, [selectedRows, allRowsBeforeFilter, sortedRows]);

  return (
    <div className="relative">
      {/* FLOATING SUMMARY BAR - Hiện khi có select */}
      {selectedSummary && selectedSummary.count > 0 && (
        <div className=" w-fit sticky top-0 z-20 mb-4 animate-in slide-in-from-top duration-300">
              ...
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">
                    Đã chọn: {selectedSummary.count} Dòng
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-xl">
                    {formatNumber(selectedSummary.totalKien)}
                  </span>
                  <span className="text-blue-100">Kiện</span>
                </div>
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          checked={isSelected}
                          onChange={(e) =>
                            onSelectRow(row._id, e.target.checked)
                          }
                          className="w-4 h-4 rounded border-slate-300 text-blue-600 cursor-pointer"
                        />
                      </td>
          ...
          <div className="px-4 py-3 bg-slate-50 border-t text-xs text-slate-600">
            <div className="flex items-center justify-between">
              <span>
                ...
                {selectedSummary && selectedSummary.count > sortedRows.length && (
                  <span className="ml-2 text-blue-600">
                    (Đã chọn {selectedSummary.count} phiếu trên tất cả dữ liệu)
                  </span>
                )}
              </span>
