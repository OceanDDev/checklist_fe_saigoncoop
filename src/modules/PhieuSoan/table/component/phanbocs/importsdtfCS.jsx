...
// components/phanboCS/importsdtfCS.jsx
...
const fetchPhanBoCSMap = async (rows) => {
  const machs = [...new Set(rows.map((r) => r.mach))];
  const results = await Promise.all(
    machs.map((mach) =>
      phanBoCSService
        .getAllPhanBoCS({ limit: 9999, mach })
        .then((res) =>
...
// ← Thêm mới: lấy slot từ DinhVi theo danh sách SKU
const fetchDinhViMap = async (rows) => {
  const skus = [...new Set(rows.map((r) => Number(r.sku)))];
  const results = await Promise.all(
    skus.map((sku) =>
      dinhViService
        .getAllDinhVi({ limit: 9999, sku })
        .then((res) => {
  ...
  for (const list of results)
    for (const dv of list)
      if (!map.has(dv.sku)) map.set(Number(dv.sku), dv.slot ?? "");
  return map;
};

// ← Sửa: loai_phieu = "SD", slot lấy từ dinhViMap
const buildPhieuPayloads = (validRows, phanboMap, dinhViMap) => {
  const grouped = new Map();
  for (const row of validRows) {
    const key = `${row.sd_tf}__${row.mach}`;
    const pb = phanboMap.get(`${row.mach}__${row.sku}__${row.ten_phan_bo}`);
    const slot = dinhViMap.get(Number(row.sku)) ?? "";
  ...
  const processFile = useCallback(
    async (file) => {
      ...
      try {
        ...
        // Step 1 — Fetch & validate vs DB (song song: phanboMap + dinhViMap)
        setStep(1);
        const [phanboMap, dinhViMap] = await Promise.all([
          fetchPhanBoCSMap(rows),
          fetchDinhViMap(rows), // ← thêm
        ]);
        ...
        // Step 2 — Import SD_TF
        setStep(2);
        const res = await phanBoCSService.importSdTfCS({ rows: validRows });

        // Step 3 — Tạo phiếu SD
        setStep(3);
        const phieuPayloads = buildPhieuPayloads(
          validRows,
          phanboMap,
          dinhViMap,
        ); // ← truyền dinhViMap
        const phieuResults = await Promise.allSettled(
          phieuPayloads.map((payload) =>
            phieuLeService.importSDPhieuLe(payload),
          ),
        );
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Cập nhật SD_TF · Tạo phiếu soạn SD
              </p>
            </div>
          </div>
          <button
...
const ImportSdTfCS = memo(({ onImportSuccess }) => {
...
ImportSdTfCS.displayName = "ImportSdTfCS";
export default ImportSdTfCS;
