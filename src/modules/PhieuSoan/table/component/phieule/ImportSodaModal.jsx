...
const ImportSodaModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          if (file.name.endsWith(".txt")) {
            await phieuLeService.importSodaTxtPhieuLe(file);
          } else {
            throw new Error("Chỉ hỗ trợ file .txt cho Soda (WPK137)");
          }
      ...
      if (results.failed === 0) {
        summaryMessage = `✅ Import thành công ${results.success} file Soda!`;
      ...
      console.error("Import Soda error:", error);
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white z-10">
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 rounded-lg bg-orange-100 text-orange-700 text-xs font-bold">
                SODA
              </span>
            ...
            <p className="text-sm text-slate-600 mt-1">
              Import file .txt từ WPK137 — chỉ hỗ trợ định dạng Soda
            </p>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="px-6 py-6 space-y-4">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {/* ✅ KHÁC: icon màu orange */}
                <svg className="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
                <h3 className="text-base font-semibold text-slate-800">Chọn file</h3>
              </div>
              {files.length > 0 && (
                <button
            ...
            {/* Dropzone - ✅ KHÁC: border orange, chỉ .txt */}
            <div className="border-2 border-dashed border-orange-200 rounded-xl p-6 hover:border-orange-400 transition-colors bg-orange-50/30">
              <input
                type="file"
                accept=".txt"
                multiple
              ...
              />
              <label
                htmlFor="soda-file-upload"
                className="flex flex-col items-center justify-center cursor-pointer"
              >
                <svg className="w-12 h-12 text-orange-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <p className="text-sm text-slate-600 mb-1">
                  {files.length > 0 ? "Click để chọn thêm file" : "Click để chọn file"}
                </p>
                {/* ✅ KHÁC: chỉ .txt */}
                <p className="text-xs text-slate-500">Chỉ hỗ trợ: .txt (WPK137) — Chọn nhiều file cùng lúc</p>
              </label>
            </div>
            ...
            {importProgress && (
              <div className="space-y-1">
                <div className="flex justify-between text-xs text-slate-600">
                  <span>Đang import file {importProgress.current}/{importProgress.total}...</span>
                  <span>{progressPercent}%</span>
                </div>
                <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                  {/* ✅ KHÁC: progress bar màu orange */}
                  <div
            ...
            {/* Import Button - ✅ KHÁC: màu orange */}
            <button
              onClick={handleImport}
              disabled={files.length === 0 || importing}
              className="w-full h-11 rounded-xl bg-gradient-to-r from-orange-500 to-pink-500 text-white font-medium hover:from-orange-600 hover:to-pink-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {importing ? (
                <>
                ...
                <>
                  ...
                  Import Soda {files.length > 0 ? `${files.length} file` : ""}
                </>
              )}
            </button>
          </div>
            ...
            <div className={`p-4 rounded-xl flex items-start gap-3 ${
              ...
              <svg className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                messageType === "success" ? "text-green-600"
                  : messageType === "error" ? "text-rose-600"
                  : "text-orange-600"
              ...
              <p className={`text-sm whitespace-pre-line ${
                messageType === "success" ? "text-green-800"
                  : messageType === "error" ? "text-rose-800"
                  : "text-orange-800"
              }`}>
                {message}
              </p>
...
export default ImportSodaModal;