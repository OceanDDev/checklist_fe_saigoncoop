...
// Hỗ trợ 2 layout:
//   Layout A (cũ):  A2 = tên phân bổ thật, SKU từ row1 col5+, data từ row6+
//   Layout B (mới): A1 = "TEN_PHAN_BO" (placeholder hoặc tên thật),
//                   SKU từ row1 col5+, TÊN HÀNG row2, Qui Cách row3, data từ row6+
const parseExcelTemplate = async (file) => {
  ...
  // ── Detect layout & lấy tên phân bổ ──────────────────────────────────────
  const a1 = String(cellVal(1, 1) || "").trim();
  const a2 = String(cellVal(2, 1) || "").trim();

  let ten_phan_bo;
  if (a1 && a1.toUpperCase() !== "TEN_PHAN_BO") {
    ten_phan_bo = a1;
  } else if (a2 && a2.toUpperCase() !== "TEN_PHAN_BO") {
    ten_phan_bo = a2;
  } else {
    ten_phan_bo =
      file.name
  ...
  // ── SKU list từ row 1 col 5+ (Do cột 4 đã dùng cho sd_tf) ─────────────────
  const skuList = [];
  const lastCol = ws.columnCount || 50;
  for (let col = 5; col <= lastCol; col++) {
    const sku = cellVal(1, col);
    if (!sku || isNaN(Number(sku))) break;
    skuList.push({
      col,
      sku: Number(sku),
      name: String(cellVal(2, col) || "").trim(),
      pack: Number(cellVal(3, col) || 1),
    });
  }

  // ── Data rows từ row 6+ ───────────────────────────────────────────────────
  // Col 2 = Mã ST, Col 3 = Tên Siêu Thị, Col 4 = sd_tf
  const items = [];
  const lastRow = ws.rowCount || 200;

  for (let r = 6; r <= lastRow; r++) {
    const mach = String(cellVal(r, 2) ?? "").trim();
    const tench = String(cellVal(r, 3) ?? "").trim();
    const sd_tf = String(cellVal(r, 4) ?? "").trim(); // Thêm field sd_tf

    if (!mach || !tench || mach === "null" || isNaN(Number(mach))) continue;

    for (const { col, sku, name, pack } of skuList) {
      const thung = Number(cellVal(r, col) ?? 0);
      ...
      items.push({
        ...
        mach,
        tench,
        sd_tf, // Truyền field vào items payload
...
// ─── Modal ────────────────────────────────────────────────────────────────────
const ImportPhanBoModal = ({ isOpen, onClose, onImportSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    setResult(null);
    try {
      const res = await phanBoService.importManyPhanBo({
        items: preview.items,
      });
      setResult({
        success: true,
  ...
  const handleDownloadTemplate = async () => {
    const workbook = new ExcelJS.Workbook();
    const ws = workbook.addWorksheet("KHO VE TINH");

    // ── Merge A1:C4 cho vùng TEN_PHAN_BO ──
    ws.mergeCells("A1:C4");
    ...
    ws.getCell("D4").value = "Tổng lượng";
    Object.assign(ws.getCell("D4"), labelStyle);
    ...
    ws.getCell("E4").value = { formula: "SUM(E6:E1000)" };
    ...
    ws.getCell("F4").value = { formula: "SUM(F6:F1000)" };
    ...
    const sampleRows = [
      [1, 9001, "BH CỬA HÀNG MẪU 1", "SD1", 2, 1], // Thêm giá trị mẫu cho SD_TF
    ...
    ws.getColumn(4).width = 16; // Width cột SD_TF
    ...
    a.href = url;
    a.download = "Template_PhanBo_LPB_CNMS.xlsx";
    a.click();
      ...
      >
        <div
          style={{
            pointerEvents: "auto",
            width: "100%",
            maxWidth: 720, // Nới rộng chút cho dễ xem thêm cột
            background: "#fff",
            borderRadius: 20,
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50 shrink-0">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Upload file Excel theo template LPB CNMS CO.OP FOOD
              </p>
            </div>
            <button
              onClick={handleClose}
              ...
              {parsing ? (
                ...
                <>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Hỗ trợ .xlsx, .xls — Template: LPB CNMS CO.OP FOOD
                  </p>
                </>
              )}
            </div>
            ...
            {preview && !error && (
              <div className="rounded-xl border border-slate-200 overflow-hidden">
                    ...
                    <thead className="bg-slate-100 sticky top-0">
                      <tr>
                        {[
                          ...
                          <th
                            key={h}
                            className={`px-3 py-2 font-semibold text-slate-600 ${h === "QC" || h === "Lượng PB" ? "text-right" : "text-left"}`}
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {preview.items.slice(0, 8).map((item, i) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-1.5 max-w-[150px] truncate text-slate-600">
                            {item.name}
                          </td>
                ...
                <div className="px-4 py-2 bg-amber-50 border-t border-amber-100 text-xs text-amber-700">
                  💡 Lượng phân bổ = Số thùng × Quy cách &nbsp;|&nbsp; Ô = 0 đã
                  được bỏ qua
                </div>
              </div>
            )}
...
// ─── Trigger Button ───────────────────────────────────────────────────────────
const ImportPhanBo = ({ onImportSuccess }) => {
