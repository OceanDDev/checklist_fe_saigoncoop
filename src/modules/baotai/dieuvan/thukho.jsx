...
// TODO: chưa có service riêng cho danh mục NV_TK trong baotai.service.js.
// Khi có API (vd: nvTkService.getAll / create / update / remove) thì nối vào
// các hàm addItem / saveEdit / removeItem bên dưới, tương tự cách baoTaiService
// đang được dùng ở component cha.
export default function NvTkModal({ items, setItems, onClose }) {
  ...
  const addItem = () => {
    ...
    if (!value) return;
    setItems((prev) => [...prev, { id: `nvtk-${Date.now()}`, name: value }]);
    setDraft("");
    // TODO: gọi API tạo NV_TK, vd: nvTkService.create({ name: value })
  };
  ...
  const saveEdit = (id) => {
    ...
    setEditingId(null);
    setEditingValue("");
    // TODO: gọi API cập nhật NV_TK, vd: nvTkService.update(id, { name: value })
  };

  const removeItem = (id) => {
    setItems((prev) => prev.filter((it) => it.id !== id));
    // TODO: gọi API xóa NV_TK, vd: nvTkService.remove(id)
  };
