...
import DataCHTable from "../PhieuSoan/table/dataCH";
...
const ViewTabs = memo(function ViewTabs({
  ...
  const tabs = [
    ...
    ...(showCuaHang
      ? [{ key: "cuaHang", label: "Cửa hàng", icon: Store }]
      : []),
  ];
  return (
...
const canDelete = useMemo(
  ...
  [],
    ...
    useEffect(() => {
      if (!canDelete && view === "cuaHang") setView("table");
    }, [canDelete, view]);
    ...
    return (
      <div className="mx-auto max-w-[1900px] space-y-4 p-4 md:p-6 bg-gradient-to-b from-slate-50 to-white min-h-screen">
          ...
          <div className="flex flex-col gap-3 rounded-2xl bg-white/80 p-4 shadow-sm ring-1 ring-slate-200 backdrop-blur md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex-shrink-0">
              <ViewTabs
                ...
                hideTable={isRole76}
                showCuaHang={canDelete} // 👈 role 57 mới thấy tab Cửa hàng
              />
            </div>
          </div>
        ...
        {view === "cuaHang" && canDelete && <DataCHTable />}
