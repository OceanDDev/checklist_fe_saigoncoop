...
const TableXuatHang = () => {
  ...
  const canManageKPI = useMemo(
    () =>
      userRoles.some((r) =>
        [
          ROLE_KPI.PGD, // 17
          ROLE_KPI.TOTRUONGXUAT1, // 13
          ROLE_KPI.TOTRUONGXUAT2, // 14
        ].includes(r),
      ),
    [userRoles],
  );
  const canScoreStaff = useCallback(
    (staff) => {
      ...
      // TOTRUONGXUAT1/2: CHỈ được chấm nhân viên Xuất hàng chính (không có đơn vị phụ)
      if (
        userRoles.some((r) =>
          [ROLE_KPI.TOTRUONGXUAT1, ROLE_KPI.TOTRUONGXUAT2].includes(r),
        )
      ) {
        return donViChinh.trim().toLowerCase() === "xuất hàng";
      }
  ...
  const MobileStaffCard = ({ item, index }) => {
    const kpiStatus = getKPIStatus(item.ma_nhan_vien);
    const yearlyWeight = getYearlyWeight(item.ma_nhan_vien);
    const donViChinh = item.don_vi?.chinh || item.don_vi || "";
    const donViPhu = item.don_vi?.phu || "";
    const hasPhu = donViPhu.trim().length > 0;

    return (
      <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3 hover:shadow-md transition-shadow">
        {/* Header */}
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="h-12 w-12 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 text-white font-bold text-lg grid place-items-center flex-shrink-0">
          ...
          <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 text-white text-xs font-bold grid place-items-center flex-shrink-0">
            {index + 1}
          </div>
        </div>
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex items-center justify-between">
            ...
            {kpiStatus.status === "completed" ? (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700">
                ...
                {kpiStatus.quarterYear} ✅
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-700">
          ...
          <div className="flex items-center justify-between">
            <span className="text-slate-500">Tỷ trọng {selectedYear}:</span>
            {yearlyWeight.averageWeight !== null ? (
              <div className="flex flex-col items-end gap-0.5">
                <span
                  className={[
                    "inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-bold ring-2",
              ...
              <span className="inline-flex items-center rounded-full bg-slate-100 text-slate-500 ring-1 ring-slate-200 px-2 py-0.5 text-xs">
                {yearStatus === "future" ? "Sắp mở" : "Chưa có"}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-100">
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 shadow-sm transition"
          >
            📊 Chi tiết
          </button>
          ...
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 shadow-sm transition"
          >
            📤 Excel
          </button>
        </div>
  ...
  return (
    <section className="container mx-auto max-w-screen-2xl p-4 md:p-6 lg:p-8 space-y-6">
      ...
      {/* Content - Mobile Cards hoặc Desktop Table */}
      {isMobile ? (
        // Mobile View - Cards
        <div className="space-y-3">
          ...
          {!loading &&
            data.length > 0 &&
            data.map((item, index) => (
              <MobileStaffCard key={item._id} item={item} index={index} />
            ))}
              ...
              <thead className="sticky top-0 z-10 bg-slate-50/90 backdrop-blur border-b border-slate-200">
                <tr>
                  {[
                    ...
                    // Tìm dòng 499, SỬA class của <th>:
                    <th
                      key={h}
                      className="px-4 md:px-5 py-3 text-left md:text-center first:text-left"
                    >
                      <span className="text-[10px] md:text-xs font-bold uppercase tracking-wide text-slate-600">
                        {h}
                      </span>
                    </th>
                  ))}
                        ...
                        <td className="px-5 py-3">
                          <div className="flex flex-wrap items-center justify-center gap-2">
                            <button
                              ...
                              className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs md:text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 shadow-sm transition whitespace-nowrap"
                            ...
                            <button
                              onClick={() => openExportModal(item)}
                              className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs md:text-sm font-medium text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 shadow-sm transition whitespace-nowrap"
